...
class Perwalian_admin_model extends CI_Model{
	...
	public function getMhsw($tahun)
	{
		$unip 		= $this->session->userdata("unip");
		$fields="(@row_number:=@row_number + 1) AS row ,_v2_mhsw.nim,_v2_mhsw.name,_v2_mhsw.TahunAkademik angkatan,_v2_statusmhsw.Nama status,sum(sks) sks";
		$where = array(
			'_v2_mhsw.DosenID'	=> $unip
		);
		$on			= '_v2_krs'.$tahun.'.NIM = _v2_mhsw.NIM';
		$group 		= '_v2_krs'.$tahun.'.NIM';
		$this->db->query("SET @row_number = 0");
		return $this->db->select($fields)
										->join('_v2_statusmhsw','_v2_statusmhsw.Kode=_v2_mhsw.status','inner')
										->join('_v2_krs'.$tahun,$on,'inner')
										->group_by($group)
										->order_by("_v2_mhsw.nim","ASC")
										->get_where('_v2_mhsw',$where)->result();
	}
