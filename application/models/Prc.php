...
class Prc extends CI_Model{
	...
	//FUNCTION DI prckrs_siakadNIM untuk ambil data krs di siakad lama berdasarkan nim dan tahun
	public function getKrsLama($nim,$tahun) {
		$data = $this->db2->query("SELECT * FROM krs".$tahun." WHERE NIM='$nim' AND Tahun='$tahun'");
		return $data;
	}

	//FUNCTION DI prckrs_siakadNIM untuk cek data krs di siakad baru berdasarkan nim dan tahun
	public function checkKrsBaru($nim,$tahun,$IDMK) {
		$data = $this->db->query("SELECT * FROM _v2_krs".$tahun." WHERE NIM='$nim' AND Tahun='$tahun' AND IDMK='$IDMK'");
		return $data->num_rows();
	}

	//FUNCTION SAVE KRS
	public function save_krs_siakad_baru($data, $tahun) {
		return ($this->db->insert('_v2_krs'.$tahun, $data)) ? TRUE : FALSE;
	}
	...
	public function getAllKHS($periode, $fak) {
		$data = $this->db->query("SELECT k.ID, k.NIM, k.Tahun, k.st_feeder, k.stprc FROM _v2_khs k LEFT JOIN _v2_mhsw m ON k.NIM = m.NIM WHERE m.KodeFakultas = '$fak' AND k.Tahun='$periode' AND k.stprc='2' AND k.st_feeder='0' AND k.error_code='' LIMIT 500");
		return $data->result();
	}

	public function getAllKRS($periode, $fak) {
		$data = $this->db->query("SELECT k.ID, k.NIM, k.Tahun, k.st_feeder, k.enkripsi FROM _v2_krs".$periode." k LEFT JOIN _v2_mhsw m ON k.NIM = m.NIM WHERE m.KodeFakultas = '$fak' AND k.Tahun='$periode' AND k.st_feeder='0' AND k.error_code='' LIMIT 500");
		return $data->result();
	}

	public function setError($id, $data) {
		return ($this->db->update('_v2_khs', $data, "ID = '$id'")) ? TRUE : FALSE;
	}
