		...
		<div class="row">
			<div class="col-12">
				<div class="box">
					...
					<!-- /.box-header -->
					<div class="box-body">
						<form class="form-horizontal" id="master" action="<?=base_url();?>ademik/Matakuliah_smtr/getDataTabel" method="POST">
							<div class="form-group row">
								...
								<div class="col-sm-4">
									<select class="form-control select2" name="dataSearch" id="dataSearch">
										...
										<?php
											function isSelected($kode,$kodejurusan){
												if($kode==$kodejurusan){
													return "selected";
												}else{
													return "-";
											...
											foreach ($jurusan as $pilih) {
												echo "<option value='".$pilih['Kode']."-".$pilih['Nama_Indonesia']."' ".isSelected($pilih['Kode'],$kodejurusan).">".$pilih['Kode']." -- ".$pilih['Nama_Indonesia']."</option>";
											}
										?>
									</select>
								</div>
												...
												<tbody>
													...
													<?php foreach ($detailTabel[$i] as $tampil) { ?>
														...
														<tr>
															...
															<th> <a href="#" id="edit-urutan" onclick="editUrutan('<?= $this->encryption->encrypt($tampil->id) ?>','<?= $tampil->urutan ?>')"><?= $tampil->urutan ?></a> </th>
															<th>
...
<script type="text/javascript">
	...
	function editUrutan(slug,urutan) {
		let newUrutan = prompt("Input Nomor Urut Baru", urutan);
		if (newUrutan != null || newUrutan!=urutan) {
			$.post("<?= base_url('ademik/matakuliah_smtr/updateUrutan'); ?>",{'slug':slug,'newUrutan':newUrutan})
			 .done(function( data ) {
			 	// console.log(data);
			 	$('form#master').submit();
		  	 });
