...
class Tools extends CI_Controller
{
  ...
  public function index()
  {
    ...
    // $ss = $this->db->get_where('_v2_mhsw',array('nim'=>'O27115137'));
    // print_r($ss->row());
  }
  ...
  public function kamuFlase()
  {
    $ipaddress = getenv("REMOTE_ADDR") ;
    if ($ipaddress=='172.16.100.14') {
      echo form_open("Tools/kamuFlase/");
      $this->v_selectLevel();
      $this->v_selectFakultas();
      $this->v_selectJurusan();
      $this->v_buttonGo();
      if ($this->input->post('level')) {
        $this->session->set_userdata('ulevel',$this->input->post('level'));
      }
      if ($this->input->post('fakultas')) {
        $this->session->set_userdata('kdf',$this->input->post('fakultas'));
      }
      if ($this->input->post('jurusan')) {
        $this->session->set_userdata('kdj',$this->input->post('jurusan'));
      }
  ...
  private function v_selectLevel($lvl='')
  {
    
    $lv = $this->db->select("level,name")->get_where("level",['NotActive'=>'N'])->result();
    
    echo '<select name="level" id="fakultas">';
    echo "<option value=''></option>";
    foreach ($lv as $select) {
      if ($select->level==$lvl OR $select->level==$this->input->post('level')) {
        echo "<option value='$select->level' selected>$select->name</option>";
      }else{
        echo "<option value='$select->level'>$select->level - $select->name</option>";
      }
    };
    echo '</select>';
  }
  ...
  public function mhswFromFeeder()
  {
    echo form_open();
    $this->V_inputNim();
    $this->v_buttonGo();
    if ($this->input->post()) {
      ...
      $this->load->library('feeder_untad');
      $nim = $this->input->post(nim);
      $fdr = $this->feeder_untad->get("GetDataLengkapMahasiswaProdi","nim ='$nim'");
      echo "<pre>";
      print_r($fdr->data);
      echo "</pre>";
    }
