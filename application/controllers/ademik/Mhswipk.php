				...
				if($getPeriodeAktif->periode_aktif==$thn){
					...
					if($show->st_feeder>0 AND $st_feederKrs==$jmlKrs){
						...
						if($thn>=20182 and ($thn!=20183 AND $thn!=20184)){
							...
							// fikri tambahkan untuk cetak KHS dari admin fakultas dan admin SU. 
							if (in_array($this->session->ulevel,[1,4,5,7])) {
								$btn .= "<a href='".base_url('ademik/report/report/cetak_khs/').$thn.'/'.$kp.'/'.$kdj.'/'.$thnAkademik.'/'.$nim.'/'.$kdf."' target='_blank'><input type='button' style='margin-top: 5px;' class='btn btn-primary' value='Cetak KHS'></a>";
							}
						}else{
							if($this->session->unip=='wawan' && $this->session->ulevel=='1' && $this->cekpjn($nim, $thn)=='false' && $kdf=='D') {
								$btn = "
