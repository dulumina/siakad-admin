...
class Mhsw_nilai extends CI_Controller {
		...
		if ($ulevel == 1){
			...
			$result = array(
	      	...
	      	if ( $typedosen == "dosenUtama"){
	        	$optmk1 = $this->app->two_val("_v2_jadwal", "IDJADWAL", $idjadwal, "NotActive", "N")->result();
	      	} else if ( $typedosen == "asstDosen") {
	        	$optmk1 = $this->app->two_val("_v2_jadwalassdsn", "IDJadwal", $idjadwal, "NotActive", "N")->result();
	      	}
	      	// print_r($optmk1);
	...
	private function input_nilai($thnkrs, $uname, $nim, $thn, $idjadwal, $ulevel, $nil) {
		...
		if (!empty($uname)){
			...
			$nama = str_replace("'","",$uname);
			$upnilai = "update _v2_krs$thnkrs set GradeNilai='$gradenilai', Bobot='$bobot', useredt='$nama', tgledt=NOW(), enkripsi='$dataEncrypt' where NIM='$nim' and Tahun='$thn' and IDJadwal='$idjadwal'";
